# -*- coding: utf-8 -*-
"""
Created on Thu Feb 16 09:44:07 2023
...
def add_0padding_crop(patch_size, overlap_size, image_path, image_path_padding, image_path_crop, pad_image=False):
    ...
    image_name = [file for file in os.listdir(image_path) if (file.endswith('.png') and not file.startswith('.'))]
    ...
    for name in image_name:
        ...
        top, bottom = math.ceil((n_0*(patch_size-overlap_size/2)-shape_0)/2), math.floor((n_0*(patch_size-overlap_size/2)-shape_0)/2)
        left, right = math.ceil((n_1*(patch_size-overlap_size/2)-shape_1)/2), math.floor((n_1*(patch_size-overlap_size/2)-shape_1)/2)
        im_pad = cv2.copyMakeBorder(im, top, bottom, left, right, cv2.BORDER_CONSTANT,value=color)
...
if os.path.exists('/home/lzhang/Desktop/FY_test//best_model_unet_plusplus_resnet34_periderm4classes_Lin_0301_1024_6.pth'):
    best_model = torch.load('/home/lzhang/Desktop/FY_test/best_model_unet_plusplus_resnet34_periderm4classes_Lin_0301_1024_6.pth', map_location=DEVICE)
    print('Loaded UNet model from this run.')

# load best saved model checkpoint from previous commit (if present)
elif os.path.exists('../input//unet-with-pretrained-resnet50-encoder-pytorch/best_model_eye_ce.pth'):
    best_model = torch.load('../input//unet-with-pretrained-resnet50-encoder-pytorch/best_model_eye_ce.pth', map_location=DEVICE)
...
sample_preds_folder = '/home/lzhang/Desktop/FY_test/Prediction_patch/'
...
image_path = "/home/lzhang/Desktop/FY_test/nature_accession/"
print(image_path)

image_path_padding = "/home/lzhang/Desktop/FY_test/Image_Padding/"
...
image_path_crop = "/home/lzhang/Desktop/FY_test/Image_Crop/" 
...
metadata_file = "/home/lzhang/Desktop/FY_test/metadata_tem.csv"
...
DATA_DIR = '/home/lzhang/Desktop/FY_test/'
pred_df = pd.read_csv('/home/lzhang/Desktop/FY_test/metadata_tem.csv')

select_classes = ['background', 'periderm', 'endodermis', 'lateral_root']

class_dict = pd.read_csv(os.path.join(DATA_DIR, 'label_class_dict_lr.csv'))
# Get class names
...
sample_preds_folder = "/home/lzhang/Desktop/FY_test/Segmentation_temp/"
if not os.path.exists(sample_preds_folder):
...
def stitch_crop_images(patch_size, overlap_size, original_image_path, image_path, stitch_path):
    ...
    for name in image_name_unique:
        ...
        #ind = 0
        im_stitch = np.zeros([int(n_0*(patch_size-overlap_size/2)), int(n_1*(patch_size-overlap_size/2)), shape_2])
        for i in range(n_0):
            for j in range(n_1):
                #ind += 1
                top = i*(patch_size-overlap_size)
                left = j*(patch_size-overlap_size)
                ind = np.squeeze(ind_array[np.where((ind_array[:,0] == i) & (ind_array[:,1]==j)),2])
                im_pred_patch = cv2.imread(image_path+name+"_"+str(ind)+".png")
                # im_crop = im_pad[top:top+patch_size, left:left+patch_size, :]
                if top == 0 and left == 0:
                    im_stitch[top:top+patch_size, left:left+patch_size, :] = im_pred_patch
                elif top == 0 and left > 0:
                    left_ind = np.squeeze(ind_array[np.where((ind_array[:,0] == i) & (ind_array[:,1]==j-1)),2])
                                       
                    im_left = cv2.imread(image_path+name+"_"+str(left_ind)+".png")
                    ...
                    im_stitch[top:top+patch_size, left:left+overlap_size, :] = np.maximum(im_pred_patch_left,im_left_right)
                    im_stitch[top:top+patch_size, left+overlap_size-1:left+patch_size-1, :] = im_pred_patch[0:patch_size, overlap_size-1:patch_size-1, :]
                    
                    
                elif top > 0 and left == 0:
                    top_ind = np.squeeze(ind_array[np.where((ind_array[:,0] == i-1) & (ind_array[:,1]==j)),2])
                    ...
                    # calculate maximum value of overlapping area
                    im_stitch[top:top+overlap_size, left:left+patch_size, :] = np.maximum(im_pred_patch_top,im_top_bottom)
                    im_stitch[top+overlap_size-1:top+patch_size-1, left:left+patch_size, :] = im_pred_patch[overlap_size-1:patch_size-1, 0:patch_size, :]
                else:
                    top_ind = np.squeeze(ind_array[np.where((ind_array[:,0] == i-1) & (ind_array[:,1]==j)),2])
                    left_ind = np.squeeze(ind_array[np.where((ind_array[:,0] == i) & (ind_array[:,1]==j-1)),2])
...
image_path = "/home/lzhang/Desktop/FY_test/Segmentation_temp/"
stitch_path =  os.path.join(DATA_DIR,'Segmentation_upp_v15/')
if not os.path.exists(stitch_path):
    os.makedirs(stitch_path)

stitch_crop_images(patch_size, overlap_size, original_image_path, image_path, stitch_path)
...
image_path = "/home/lzhang/Desktop/FY_test/Segmentation_upp_v15/"
save_path = "/home/lzhang/Desktop/FY_test/Post_processing_v09/"
...
for i in range(len(imageList)):
    ...
    contours, hierarchy = cv2.findContours(inverted_binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
    ...
    for j in range(len(contours_filter)):
        center_x, center_y = get_average(np.squeeze(contours_filter[j])) # the most accurate way is tosort based on a range of y instead of exact y value of center point
    ...
    contours_filter_list = sorted(contours_filter_list, key=lambda item:item[3], reverse=False)
    ...
    for k in range(len(contours_filter_sort)):
        for j in range(k,len(contours_filter_sort)):
            contour1, contour2 = [], []
            contour1, contour2 = np.squeeze(contours_filter_sort[k]), np.squeeze(contours_filter_sort[j])
            ...
            if contour1_x < contour2_x: # contour1 at left
                ...
                if (left2_x - right1_x)<150 and (left2_x - right1_x)>=0 and abs(contour2_y-contour1_y)<150: # original is 150, 50
                    ...
                    #segmentation_revert = image
                    segmentation_bbox = segmentation_revert[bbox_buffer[1]:bbox_buffer[1]+bbox_buffer[3],bbox_buffer[0]:bbox_buffer[0]+bbox_buffer[2],:]
                    segmentation_bbox[((segmentation_bbox[:,:,0] == 128) & (segmentation_bbox[:,:,1] == 0) & (segmentation_bbox[:,:,2] == 0))] = [128,128,0]
                    segmentation_bbox[((segmentation_bbox[:,:,0] == 0) & (segmentation_bbox[:,:,1] == 128) & (segmentation_bbox[:,:,2] == 0))] = [128,128,0]
                    segmentation_revert[bbox_buffer[1]:bbox_buffer[1]+bbox_buffer[3],bbox_buffer[0]:bbox_buffer[0]+bbox_buffer[2],:]=segmentation_bbox
                    # cv2.imwrite("Y:/Lin_Wang/FY_training/image_revert.png",segmentation_revert)  
                elif (left2_x - right1_x)>149 and abs(contour2_y-contour1_y)<1000: #1000 # and contour1_area<contour2_area/2
                    #bbox = [int(right1_x), np.min([contour2_y,contour1_y]), int(abs(right1_x- left2_x)), int(abs(contour2_y-contour1_y))]
                    left, top = np.percentile(contour1[:,0],1), np.percentile(contour1[:,1],1)
                    right, bottom = np.percentile(contour1[:,0],99), np.percentile(contour1[:,1],99)
                    ...
                    #segmentation_revert = image
                    segmentation_bbox = segmentation_revert[bbox_buffer[1]:bbox_buffer[1]+bbox_buffer[3],bbox_buffer[0]:bbox_buffer[0]+bbox_buffer[2],:]
                    segmentation_bbox[((segmentation_bbox[:,:,0] == 128) & (segmentation_bbox[:,:,1] == 128) & (segmentation_bbox[:,:,2] == 0))] = [128,0,0] # [128,0,0]
...
#%% convert periderm 
image_path = "/home/lzhang/Desktop/FY_test/Post_processing_v09/"
save_path = "/home/lzhang/Desktop/FY_test/segmentation_upp_periderm_v04/"
...
#%% closing
image_path = "/home/lzhang/Desktop/FY_test/segmentation_upp_periderm_v04/"
save_path_1 = "/home/lzhang/Desktop/FY_test/Final_Periderm_Segmentation_Results/"
