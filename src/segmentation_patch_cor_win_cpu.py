...
def add_0padding_crop(patch_size, overlap_size, image_path, image_path_padding, image_path_crop, pad_image=False):
    ...
    for name in image_name:
        ...
        if pad_image:
            name_pad = os.path.join(image_path_padding, name)  # Use os.path.join here as well
...
class BuildingsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        # In Windows, split path using os.path.basename to get the filename
        names = os.path.basename(self.image_paths[i]).split('.')[0]
...
def load_model(model_path):
    ...
    # Remove DataParallel wrapper if model was saved with it
...
# If not, check for an alternative model file
    ...
    best_model = load_model(alt_model_path)
...
#%% 
for idx in range(len(test_dataset)):  # You could also limit this to a smaller range for testing
    print(idx)
    try:
        ...
        # If the model returns a single class channel for the foreground, check if it's not empty
        foreground_prediction = pred_mask[:, :, select_classes.index('periderm')]
        if np.sum(foreground_prediction) == 0:
            print(f"Warning: Empty prediction for index {idx} named {names}.")
        ...
        save_path = os.path.join(sample_preds_folder, f"{names}.png")
        cv2.imwrite(save_path, pred_mask)
        print(f"Saved prediction for index {idx} named {names}.")
        
    except Exception as e:
        print(f"Error processing index {idx} named {names}: {e}")
