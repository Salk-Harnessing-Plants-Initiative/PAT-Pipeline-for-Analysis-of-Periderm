...
def process_image(image_path):
    ...
    # remove the closed loop skeleton
    skeleton = skeleton_int.copy()

    # Helper function to count endpoints
    def count_endpoints(component):
        kernel = np.ones((3, 3))
        convolved = convolve(component.astype(int), kernel, mode="constant", cval=0)
        endpoints = component & (convolved == 2)
        return np.count_nonzero(endpoints)

    # Label connected components
    labeled_skeleton = label(skeleton)

    # Prepare clean skeleton
    clean_skeleton = np.zeros_like(skeleton, dtype=bool)

    # Process each component
    for region in regionprops(labeled_skeleton):
        coords = region.coords
        component = np.zeros_like(skeleton, dtype=bool)
        component[tuple(coords.T)] = True

        if count_endpoints(component) >= 2:
            clean_skeleton |= component  # keep open structures

    # Now use clean_skeleton in FilFinder
    skeleton_int_clean = clean_skeleton
    skeleton_int_clean = (skeleton_int_clean * 255).astype(np.uint8)

    start_time = time.time()
    # fil = FilFinder2D(skeleton_int, distance=250 * u.pc, mask=skeleton_int)
    mask = skeleton_int_clean > 0
    fil = FilFinder2D(skeleton_int_clean, distance=10 * u.pc, mask=mask)
    fil.preprocess_image(flatten_percent=85)
    fil.create_mask(border_masking=True, verbose=False, use_existing_mask=True)
    fil.medskel(verbose=False)

    fil.analyze_skeletons(
        branch_thresh=40 * u.pix, skel_thresh=10 * u.pix, prune_criteria="length"
    )
    end_time = time.time()
...
def main():
    input_folder = os.path.join(
        main_dir, "output", "WB_whole_root"
    )  # Source folder for images
    output_file = os.path.join(
        main_dir, "output", "whole_root_length_pixels.csv"
    )  # CSV file for output
