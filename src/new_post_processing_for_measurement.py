...
def post_process_images(final_image, image2_path):
    image2 = cv2.imread(image2_path)
    if image2 is None or final_image is None:
...
def keep_right_remove_left(final_image, colored_version, output_path):
    ...
    print("Final image saved to:", output_path)

def main():
    folder1 = os.path.join(main_dir, 'output','Segmentation_upp_v15')
    folder2 = os.path.join(main_dir, 'output','Final_Periderm_Segmentation_Results')
    output_folder = os.path.join(main_dir, 'output','measurement')
    ...
    for filename in os.listdir(folder1):
        ...
        if os.path.isfile(image_path2):
            final_image = process_images(image_path1)
            colored_version = post_process_images(final_image, image_path2)
            colored_version = red_to_white(colored_version)
            colored_version = red_to_black(colored_version)
            keep_right_remove_left(final_image, colored_version, output_path)
